...
unsigned int udpRemotePort = 8175;
...
void fncUdpSend()
{
	...
	packet.Packet.Values[0] = (double)temp;

	temp = mySensor.readFloatPressure();

	Serial.print("Pressure: ");
	Serial.print(temp, 2);
	Serial.println(" Pa");

	packet.Packet.Values[1] = (double)temp;

	temp = mySensor.readFloatAltitudeMeters();
	Serial.print("Altitude: ");
	Serial.print(temp, 2);
	Serial.println("m");

	packet.Packet.Values[2] = (double)temp;

	temp = mySensor.readFloatHumidity();

	Serial.print("%RH: ");
	Serial.print(temp, 2);
	Serial.println(" %");
	packet.Packet.Values[3] = (double)temp;

	Serial.println();

	packet.Packet.DeviceId = 10;
	packet.Packet.SensorType = SensorTypes::TemperatureHumidityPresure;
